...
:- module(lab_server, [start_server/1, handle_abnormal/1, handle_get_tests/1, abnormal/5, normalize_test_name/2, find_normal_range_with_fallback/6, with_patient_and_tests/5]).

:- use_module(library(http/thread_httpd)).
:- use_module(library(http/http_dispatch)).
:- use_module(library(http/http_json)).
:- use_module(library(http/parameters)).
...
% Utility for encapsulated test contexts
